...
function renderSubCategoryOptions() {
  renderSubcategoryOptions();
  renderSubcategoryTrigger();
  const total = document.getElementById('subcategory-pool-total');
  if (total) total.textContent = subCategoriesData.length;
  // 列表页筛选的子品类
...
function getSubcategoryLabel(value) {
  if (!value) return '';
  const item = subCategoriesData.find(s => s.value === value);
  return item ? item.label : ((typeof subLabels !== 'undefined' && subLabels[value]) || value);
}

let subcategorySearchQuery = '';

function renderSubcategoryTrigger() {
  const text = document.getElementById('subcategory-trigger-text');
  const hidden = document.getElementById('subcategory');
  if (!text || !hidden) return;
  const val = hidden.value;
  if (val) {
    text.className = 'sku-trigger-summary';
    text.textContent = getSubcategoryLabel(val);
  } else {
    text.className = 'sku-trigger-placeholder';
    text.textContent = '请选择子品类';
  }
}

function renderSubcategoryOptions() {
  const list = document.getElementById('subcategory-dropdown-list');
  if (!list) return;
  const q = subcategorySearchQuery.toLowerCase();
  const filtered = subCategoriesData.filter(s =>
    !q || s.label.toLowerCase().includes(q) || String(s.value).toLowerCase().includes(q)
  );
  const current = document.getElementById('subcategory')?.value || '';
  if (!filtered.length) {
    list.innerHTML = '<div style="padding:16px;text-align:center;color:var(--text-muted);font-size:13px;">无匹配子品类</div>';
    return;
  }
  list.innerHTML = filtered.map(s => `
    <div class="sku-option ${current === s.value ? 'selected' : ''}" onclick="selectSubcategory('${s.value}')">
      <span class="sku-option-radio"></span>
      <div class="sku-option-info"><div class="sku-option-name">${s.label}</div></div>
    </div>
  `).join('');
}
...
function selectSubcategory(value) {
  ...
  const search = document.getElementById('subcategory-search');
...
function toggleSubcategoryDropdown(e) {
  ...
  if (!dropdown) return;
  if (dropdown.classList.contains('show')) closeSubcategoryDropdown();
  else openSubcategoryDropdown();
}

function openSubcategoryDropdown() {
  closeSkuDropdown();
  document.getElementById('subcategory-dropdown').classList.add('show');
  setTimeout(() => {
    const search = document.getElementById('subcategory-search');
...
function filterSubcategoryOptions() {
  subcategorySearchQuery = document.getElementById('subcategory-search').value.trim().toLowerCase();
  renderSubcategoryOptions();
}
...
document.addEventListener('click', (e) => {
  const wrap = document.querySelector('.sku-select-wrap:not(.subcategory-select-wrap)');
  if (wrap && !wrap.contains(e.target)) closeSkuDropdown();
  const subWrap = document.querySelector('.subcategory-select-wrap');
  if (subWrap && !subWrap.contains(e.target)) closeSubcategoryDropdown();
});
...
function validateStep1() {
  ...
  fields.forEach(f => {
    ...
    if (f.id === 'subcategory') {
      const trigger = document.getElementById('subcategory-trigger');
  ...
  if (isDesignDeptEnabled()) {
    const designDate = document.getElementById('design-delivery-date');
    if (!designDate || !designDate.value) {
      if (designDate) designDate.classList.add('error');
      document.getElementById('design-delivery-date-error').classList.add('show');
      valid = false;
    }
  }
  return valid;
...
function checkDateConsistency() {
  const launch = document.getElementById('product-launch-date');
  const delivery = document.getElementById('delivery-date');
  const warn = document.getElementById('date-consistency-warn');
  if (!launch || !delivery || !warn) return;
  const launchVal = launch.value;
  const deliveryVal = delivery.value;
  if (launchVal && deliveryVal && launchVal < deliveryVal) {
    warn.style.display = 'block';
  } else {
    warn.style.display = 'none';
  }
  checkDesignDateConsistency();
}

function isImageCopyDemandWizard() {
  return currentBiz === 'listing7';
}

function isDesignDeptEnabled() {
  return designDeptEnabled;
}

function toggleDesignDept() {
  designDeptEnabled = !designDeptEnabled;
  const btn = document.getElementById('design-dept-toggle');
  const fields = document.getElementById('design-dept-fields');
  if (btn) btn.classList.toggle('active', designDeptEnabled);
  if (fields) fields.style.display = designDeptEnabled ? '' : 'none';
  if (!designDeptEnabled) {
    const input = document.getElementById('design-delivery-date');
    if (input) {
      input.value = '';
      clearError(input);
    }
    const warn = document.getElementById('design-date-warn');
    if (warn) warn.style.display = 'none';
  }
}

function updateDesignDeptField() {
  const group = document.getElementById('design-dept-group');
  if (!group) return;
  const show = isImageCopyDemandWizard();
  group.style.display = show ? '' : 'none';
  if (!show) {
    designDeptEnabled = false;
    const btn = document.getElementById('design-dept-toggle');
    const fields = document.getElementById('design-dept-fields');
...
function checkDesignDateConsistency() {
  const design = document.getElementById('design-delivery-date');
  const delivery = document.getElementById('delivery-date');
  const warn = document.getElementById('design-date-warn');
  if (!design || !delivery || !warn) return;
  if (design.value && delivery.value && design.value > delivery.value) {
    warn.style.display = 'block';
  } else {
    warn.style.display = 'none';
  }
...
function getDesignDeliveryDate() {
  const el = document.getElementById('design-delivery-date');
...
function syncReqType() {
  ...
  updateOptImageHint();
  updateDesignDeptField();
}

function isOptimizationImageDemandWizard() {
  return currentStage === 'old' && currentBiz === 'listing7';
}

function updateOptImageHint() {
  const hint = document.getElementById('opt-image-hint');
  if (!hint) return;
  hint.style.display = isOptimizationImageDemandWizard() ? '' : 'none';
}
...
function buildSummaryTags() {
  ...
  const subText = subValue ? getSubcategoryLabel(subValue) : '—';
  const items = [
  ...
  if (isDesignDeptEnabled()) {
    const designDate = getDesignDeliveryDate() || '—';
    items.push({ label: `🎨 设计 ${designDate}`, color: '#0f766e', bg: '#ccfbf1' });
  }
