...
function openAiChat(sku, name, opts = {}) {
  ...
  loadCustomAiSkills();
  // 写入上下文
  const contextRow = opts.row || (opts.rejectContext && opts.rejectContext.row) || null;
  aiChatState.sku = sku || aiChatState.sku || 'PO17X4011';
  aiChatState.productName = name || aiChatState.productName || '7格便携药盒';
  aiChatState.row = contextRow || null;
  aiChatState.requestType = contextRow ? (contextRow.type || '新品 Listing') : '新品 Listing';
  aiChatState.brand = contextRow ? (contextRow.brand || 'AUVON') : 'AUVON';
  aiChatState.site = contextRow ? (contextRow.site || 'US') : 'US';
  ...
  return [
    {
      ...
      score: 88 + scoreShift,
      content: c1,
      htmlContent: buildListingPointsHtml(c1),
    ...
    },
    {
      ...
      score: 91 - scoreShift,
      content: c2,
      htmlContent: buildListingPointsHtml(c2, { highlightSeoTag: '*' }),
    ...
    },
    {
      ...
      content: c3,
      htmlContent: buildListingPointsHtml(c3),
      reasons: ['表达更自然，适合美区用户阅读', '合规风险较低，避免医疗功效承诺', 'SEO 强度略低，后续可补关键词'],
    },
  ];
}

function buildImageCopyRichTextBlocks(versionKey, name, brand, site) {
  const variants = {
    v1: [
      {
        headline: '一周用药，清晰规划',
        productPoint: `7 格分区帮助用户把一周药物提前整理好，减少漏服焦虑；${brand} ${name} 让每一天用药都清晰可见。`,
      ...
      {
        headline: '大容量不笨重',
        productPoint: '大容量分格兼顾多药收纳和日常便携，适合维生素、鱼油与日常药片组合存放。',
      ...
      {
        headline: '带得出门的安心感',
        productPoint: '稳固卡扣与紧凑结构适合放入包内，通勤、旅行、露营都能保持药片有序不散落。',
    ...
    v2: [
      {
        headline: 'Your Week, Clearly Mapped',
        productPoint: 'Weekly planning made simple — each day gets its own clearly marked section so you always know what to take.',
        designRequirement: 'Link to Image 2: overhead weekly layout with day labels',
        imageCopy: 'Your Week, Organized — A clear 7-day layout helps you stay on track with vitamins, supplements and daily meds.',
      },
      {
        headline: 'Fits Real Pill Sizes',
        productPoint: 'Spacious slots fit large capsules like fish oil and multivitamins — no cramming, no crushed pills.',
        designRequirement: 'Link to Image 6: hand-held size comparison shot',
        imageCopy: 'Fits What You Actually Take — Generous compartments designed for real-life pill sizes.',
      },
      {
        headline: 'Made for Daily Carry',
        productPoint: 'Built for ' + site + ' shoppers who need a reliable travel companion — secure latch, compact footprint.',
        designRequirement: 'Link to Image 3: latch close-up in a travel bag scene',
        imageCopy: 'Travel-Ready Storage — Compact enough for your daily bag, secure enough for the commute.',
      },
    ],
    v3: [
      {
        headline: '比标准分格更能装',
        productPoint: 'Unlike flimsy generic organizers, this box uses reinforced dividers that keep pills separated week after week.',
        designRequirement: '建议关联图2：与竞品分格容量对比',
        imageCopy: 'More Room Than Standard Organizers — See the difference in compartment depth and weekly capacity.',
      },
      {
        headline: `${brand} 专业品质背书`,
        productPoint: `${brand} 专业Healthcare品牌背书，区别于无标识低价竞品，提升Listing专业可信度。`,
        designRequirement: '建议关联图4：品牌Logo与质感特写',
        imageCopy: `Professional Quality by ${brand} — Not just another pill box; built for health-conscious households.`,
      },
      {
        headline: '双卡扣防洒升级',
        productPoint: 'Dual-latch anti-spill design verified for daily carry — a practical upgrade over single-hinge alternatives.',
        designRequirement: '建议关联图3：双卡扣结构对比示意',
        imageCopy: 'Dual-Latch Security — Engineered to stay closed when life gets bumpy.',
      },
    ],
  };
  return variants[versionKey] || variants.v1;
}

function buildImageCopyRichTextHtml(blocks) {
  const cnNum = ['一', '二', '三'];
  const cards = blocks.map((b, i) => `<div class="copy-draft-img-point">
    <span class="img-point-idx">富文本${cnNum[i] || (i + 1)}</span>
    <div class="img-point-body">
      <div class="img-point-label">${b.headline}</div>
      <div class="img-point-text">${b.imageCopy}</div>
    </div>
  </div>`).join('');
  return `<div class="copy-draft-img-points">${cards}</div>`;
}

function buildImageCopyRichTextPlain(blocks) {
  const cnNum = ['一', '二', '三'];
  return blocks.map((b, i) => `富文本${cnNum[i] || (i + 1)}：${b.headline}\n${b.imageCopy}`).join('\n\n');
}
...
function buildImageCopyDraftCandidates(name, brand, site, scoreShift) {
  ...
  const versionMeta = [
    { id: 'img-conversion', key: 'v1', title: '转化导向版', tag: '强调用户利益', score: 90 + scoreShift, reasons: ['7 个卖点均直击用户痛点，转化效果预期较好', '卖点表达清晰，适合快速上线', '可结合 A+ 页面进一步丰富场景'] },
    { id: 'img-localized', key: 'v2', title: '本地化表达版', tag: '自然英文表达', score: 87 - scoreShift, reasons: ['语言自然地道，符合 ' + site + ' 站点用户阅读习惯', '合规风险较低，审核通过率高', '中英混合时可在对话中调整语言比例'] },
    { id: 'img-diff', key: 'v3', title: '差异化卖点版', tag: '突出竞品对比', score: 85 + (scoreShift > 1 ? 1 : 0), reasons: ['突出竞品差异点，有助于建立品牌认知', '适合与竞品对比图搭配使用', '部分表达较激进，需确认合规'] },
  ];

  return versionMeta.map(v => {
    const richBlocks = buildImageCopyRichTextBlocks(v.key, name, brand, site);
    return {
      id: v.id,
      title: v.title,
      tag: v.tag,
      score: v.score,
      content: buildContent(v.key),
      htmlContent: buildHtml(v.key),
      richTextContent: buildImageCopyRichTextPlain(richBlocks),
      richTextHtmlContent: buildImageCopyRichTextHtml(richBlocks),
      reasons: v.reasons,
    };
  });
}
...
function renderCopyDraftPicker() {
  ...
  if (isVideo) {
    ...
    if (hint) {
      const contentLabel = copyDraftPickerState._imageContentTab === 'richtext' ? ' · 富文本' : ' · 图片文案';
      hint.textContent = `当前选中：V${idx + 1} ${activeDraft.title}${activeDraft.richTextHtmlContent ? contentLabel : ''}`;
    }
  } else {
...
function isImageCopyDraftPicker(drafts) {
  return drafts.length > 0 && String(drafts[0].id || '').startsWith('img-');
}

function renderImageDraftContentTabs(draft) {
  const tab = copyDraftPickerState._imageContentTab || 'image';
  const imgCls = tab === 'image' ? 'active' : '';
  const rtCls = tab === 'richtext' ? 'active' : '';
  return `<div class="copy-draft-content-tabs">
    <button type="button" class="copy-draft-content-tab ${imgCls}" data-content-tab="image" onclick="switchImageDraftContentTab('image')">图片文案 (7)</button>
    <button type="button" class="copy-draft-content-tab ${rtCls}" data-content-tab="richtext" onclick="switchImageDraftContentTab('richtext')">富文本 (3)</button>
  </div>`;
}

function renderDraftPreviewHtml(draft) {
  const tab = copyDraftPickerState._imageContentTab || 'image';
  if (isImageCopyDraftPicker([draft]) && tab === 'richtext' && draft.richTextHtmlContent) {
    return draft.richTextHtmlContent;
  }
...
function renderVideoDraftTabs(drafts, esc) {
  ...
  `).join('');
  const imageCopyMode = isImageCopyDraftPicker(drafts);
  const contentTabs = imageCopyMode ? renderImageDraftContentTabs(draft) : '';
  ...
  return `
    <div class="copy-draft-tabs">
      ...
      <div class="copy-draft-tab-panel">
        ${contentTabs}
        <div class="copy-draft-preview has-html copy-draft-preview-scroll">${renderDraftPreviewHtml(draft)}</div>
...
function switchImageDraftContentTab(tab) {
  copyDraftPickerState._imageContentTab = tab;
  const panel = document.querySelector('#copy-draft-body .copy-draft-preview');
  const drafts = copyDraftPickerState.drafts;
  const activeId = copyDraftPickerState._videoTab || (drafts[0] && drafts[0].id);
  const draft = drafts.find(d => d.id === activeId) || drafts[0];
  if (panel && draft) {
    panel.innerHTML = renderDraftPreviewHtml(draft);
  }
  document.querySelectorAll('#copy-draft-body .copy-draft-content-tab').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.contentTab === tab);
  });
  const hint = document.getElementById('copy-draft-foot-hint');
  if (hint && draft) {
    const idx = drafts.indexOf(draft);
    const contentLabel = tab === 'richtext' ? ' · 富文本' : ' · 图片文案';
    hint.textContent = `当前选中：V${idx + 1} ${draft.title}${draft.richTextHtmlContent ? contentLabel : ''}`;
  }
}
...
function appendSelectedCopyDraft(draft, row) {
  ...
  let text = `已选择：${draft.title} · AI 自评分 ${draft.score}\n人工评分：${rating}\n\n${draft.content}`;
  if (draft.richTextContent) {
    text += `\n\n【富文本】\n${draft.richTextContent}`;
  }
  ...
  const outputBody = draft.richTextContent
    ? `${draft.content}\n\n【富文本】\n${draft.richTextContent}`
    : draft.content;
...
function isAiImageCopyRequest() {
  return /图片文案/.test(aiChatState.requestType || '');
}
...
function getAiImageCopySubmitItems(messageText) {
  ...
  let match;
  while ((match = blockReg.exec(raw)) && parsed.length < 7) {
    const n = numMap[match[1]] || Number(match[1]) || (parsed.length + 1);
    const image = n === 1 ? '主图' : `图${n}`;
    parsed.push({
      image,
...
function getAiImageCopyRichTextDefault() {
  const ic = (typeof MOCK_DATA !== 'undefined' && MOCK_DATA.imageCreative) ? MOCK_DATA.imageCreative : {};
  const blocks = Array.isArray(ic.richTextBlocks) && ic.richTextBlocks.length
    ? ic.richTextBlocks
    : (ic.richText || []).map(it => ({ title: it.title, productPoint: it.content }));
  return blocks.map(item => {
    const title = item.title || '富文本模块';
    const content = item.productPoint || item.content || '';
    return `${title}\n${content}`.trim();
  }).filter(Boolean).join('\n\n');
}

const AI_SUBMIT_TEXT_TAGS = ['新增', '调整'];

function aiSubmitTagFieldKey(field) {
  return encodeURIComponent(field).replace(/%/g, '_');
}

function getAiSubmitTagClass(tag) {
  const map = {
    '新增': 'add',
    '调整': 'edit',
    '修改': 'edit',
...
function normalizeAiSubmitTagsForText(text, tags) {
...
function renderAiSubmitOverlayText(text, tags) {
  ...
  let html = '';
  validTags.forEach(item => {
    html += `<span class="aichat-annotated-ghost">${escapeAiHtml(content.slice(cursor, item.start))}</span>`;
    html += `<mark class="aichat-inline-tag aichat-inline-tag-${getAiSubmitTagClass(item.tag)}"><sup>${escapeAiHtml(item.tag)}</sup><span>${escapeAiHtml(content.slice(item.start, item.end))}</span></mark>`;
    cursor = item.end;
  });
  html += `<span class="aichat-annotated-ghost">${escapeAiHtml(content.slice(cursor))}</span>`;
  return `<div class="aichat-annotated-layer-inner">${html}</div>`;
}

function syncAiSubmitTagOverlayScroll(field, textareaId) {
  const overlay = document.getElementById(`submit-tag-overlay-${aiSubmitTagFieldKey(field)}`);
  const el = document.getElementById(textareaId);
  if (!overlay || !el) return;
  const inner = overlay.querySelector('.aichat-annotated-layer-inner');
  if (inner) {
    inner.style.transform = `translate(${-el.scrollLeft}px, ${-el.scrollTop}px)`;
    overlay.dataset.scrollTop = String(el.scrollTop);
    overlay.dataset.scrollLeft = String(el.scrollLeft);
  }
}
...
function renderAiSubmitTagPreview(field, textareaId) {
  ...
  if (overlay) {
    ...
    syncAiSubmitTagOverlayScroll(field, textareaId);
  }
}

function renderAiSubmitTagList(field) {
  const list = document.getElementById(`submit-tag-list-${aiSubmitTagFieldKey(field)}`);
  if (!list) return;
  const tags = (aiSubmitModalState.textTags || []).filter(item => item.field === field);
  list.innerHTML = tags.length
    ? `<div class="aichat-submit-tag-list-title">已标注</div>${tags.map(item => `
        <span class="aichat-submit-tag-chip">
          <b>${escapeAiHtml(item.tag)}</b>${escapeAiHtml(item.text)}
          <button type="button" onclick="removeAiSubmitTextTag('${item.id}')" title="移除">×</button>
        </span>
      `).join('')}`
    : '';
}

function addAiSubmitTextTag(field, textareaId, tag) {
  const el = document.getElementById(textareaId);
  if (!el) return;
  ...
  if (!text) {
    showToast('请先选中需要标注的文字', 'warning');
    el.focus();
    return;
  }
  aiSubmitModalState.textTags = aiSubmitModalState.textTags || [];
  aiSubmitModalState.textTags.push({
  ...
  renderAiSubmitTagList(field);
  renderAiSubmitTagPreview(field, textareaId);
  el.focus();
}

function removeAiSubmitTextTag(id) {
  const item = (aiSubmitModalState.textTags || []).find(tag => tag.id === id);
  aiSubmitModalState.textTags = (aiSubmitModalState.textTags || []).filter(tag => tag.id !== id);
  if (item) {
...
function collectAiSubmitTextTags(fieldTextMap) {
  const result = [];
  Object.keys(fieldTextMap || {}).forEach(field => {
    const tags = (aiSubmitModalState.textTags || []).filter(item => item.field === field);
    normalizeAiSubmitTagsForText(fieldTextMap[field], tags).forEach(item => {
      result.push({
        field: item.field,
        tag: item.tag,
        text: item.text,
...
function openAiSubmitModal(messageId) {
  const m = aiChatState.messages.find(x => x.id === messageId);
  if (!m) return;
  const isImageCopy = isAiImageCopyRequest();
  aiSubmitModalState = { sourceMessageId: messageId, mode: isImageCopy ? 'imageCopy' : 'listing', textTags: [] };
  const root = ensureAiSubmitModal();
  const body = document.getElementById('aichat-submit-modal-body');
  if (!body) return;
  const hint = root.querySelector('.aichat-submit-modal-head p');
  if (hint) {
    hint.textContent = isImageCopy
      ? '确认提交前可编辑每张图的卖点内容和图片文案'
      : '确认提交前可自由编辑 Title 和 5 条 TD';
  }
  const requestType = aiChatState.requestType || (isImageCopy ? '新品图片文案' : '新品 Listing');
  const contextHtml = `
    <div class="aichat-submit-context">
      <span>需求类型：${escapeAiHtml(requestType)}</span>
      <span>SKU：${escapeAiHtml(aiChatState.sku || 'PO17X4011')}</span>
      <span>产品：${escapeAiHtml(aiChatState.productName || '7格便携药盒')}</span>
    </div>`;

  if (isImageCopy) {
    const items = getAiImageCopySubmitItems(m.text || '');
    body.innerHTML = contextHtml + `
      <div class="aichat-submit-image-list">
        ${items.map((item, idx) => `
          <div class="aichat-submit-image-card">
            <div class="aichat-submit-image-head">
              <strong>${escapeAiHtml(item.image || (idx === 0 ? '主图' : `图${idx + 1}`))}</strong>
    ...
    body.innerHTML = contextHtml + `
      <div class="aichat-submit-field">
        ...
        <div class="aichat-annotated-textarea-wrap">
...
function confirmAiSubmitReview() {
  if (aiSubmitModalState.mode === 'imageCopy') {
    ...
    const imageCopies = cards.map((card, idx) => {
    ...
    if (!imageCopies.length || firstEmpty >= 0) {
      showToast(`请先填写${imageCopies[firstEmpty] ? imageCopies[firstEmpty].image : '图片'}的卖点内容或图片文案`, 'warning');
      const target = document.getElementById(`submit-image-point-${Math.max(0, firstEmpty)}`);
    ...
    const fieldTextMap = imageCopies.reduce((acc, item) => {
      ...
      return acc;
    }, { '富文本': richTextEl ? richTextEl.value.trim() : '' });
    const sourceId = aiSubmitModalState.sourceMessageId;
    const textTags = collectAiSubmitTextTags(fieldTextMap);
    closeAiSubmitModal();
    submitAiMessage(sourceId, {
      type: 'imageCopy',
      imageCopies,
      richText: richTextEl ? richTextEl.value.trim() : '',
      richTextNote: richTextNoteEl ? richTextNoteEl.value.trim() : '',
      textTags,
    });
    return;
  }
  ...
  const titleNoteEl = document.getElementById('submit-title-note');
  const tds = Array.from({ length: 5 }, (_, i) => {
    const el = document.getElementById(`submit-td-${i}`);
    return el ? el.value.trim() : '';
  });
  const tdNotes = Array.from({ length: 5 }, (_, i) => {
    const el = document.getElementById(`submit-td-note-${i}`);
    return el ? el.value.trim() : '';
  });
  if (!title) {
  ...
  submitAiMessage(sourceId, {
    type: 'listing',
    title,
    titleNote: titleNoteEl ? titleNoteEl.value.trim() : '',
    tds,
    tdNotes,
    textTags,
  });
...
function getAiSubmitNoteKey() {
  return [aiChatState.sku || '', aiChatState.requestType || '', aiChatState.productName || ''].join('|');
}

function cacheAiSubmitNotes(payload) {
  if (!payload) return;
  window.AI_SUBMIT_REVIEW_NOTES = window.AI_SUBMIT_REVIEW_NOTES || {};
  window.AI_SUBMIT_REVIEW_NOTES[getAiSubmitNoteKey()] = payload;
}
...
function submitAiMessage(messageId, payload = null) {
  ...
  if (payload && payload.type === 'imageCopy') {
    ...
    text += `\n\n【Title】\n${payload.title}${payload.titleNote ? `\n\n修改说明：${payload.titleNote}` : ''}\n\n${payload.tds.map((td, i) => `【TD-${i + 1}】\n${td}${payload.tdNotes && payload.tdNotes[i] ? `\n\n修改说明：${payload.tdNotes[i]}` : ''}`).join('\n\n')}`;
  }
  if (payload && payload.textTags && payload.textTags.length) {
    text += `\n\n【标注内容】\n${payload.textTags.map(item => `[${item.tag}] ${item.field}：${item.text}`).join('\n')}`;
  }
...
function renderRowActions(r, rowIdx) {
  ...
  switch (r.status) {
    case '待审核':
      return `<div class="row-actions"><button class="row-action-btn warn" onclick="rowAction('change','${sku}',${idx})">变更</button>${div}<button class="row-action-btn danger soft" onclick="rowAction('withdraw','${sku}',${idx})">撤回</button></div>`;
    case '已驳回':
...
function rowAction(act, sku, rowIdx) {
  if (act === 'withdraw') {
    openWithdrawDemandModal(rowIdx, sku);
    return;
  }
  if (act === 'view_copy') {
...
let _withdrawRowRef = null;

function openWithdrawDemandModal(rowIdx, sku) {
  const row = (rowIdx >= 0 && LIST_DATA[rowIdx])
    ? LIST_DATA[rowIdx]
    : LIST_DATA.find(r => r.sku === sku && r.status === '待审核');
  if (!row) {
    showToast('未找到可撤回的需求', 'warning');
    return;
  }
  if (row.status !== '待审核') {
    showToast('只有待审核需求可以撤回', 'warning');
    return;
  }
  _withdrawRowRef = row;
  const modal = document.getElementById('withdraw-demand-modal');
  if (!modal) return;
  document.getElementById('withdraw-demand-subtitle').textContent = `${row.type} · ${row.sku}`;
  document.getElementById('withdraw-demand-summary').innerHTML = `
    <div><span>产品</span><strong>${row.brand} · ${row.name}</strong></div>
    <div><span>站点 / 子品类</span><strong>${row.site} · ${row.sub}</strong></div>
    <div><span>提交人 / 文案人员</span><strong>${row.op || '—'} · ${row.writer || '—'}</strong></div>
    <div><span>期望交付</span><strong>${row.date || '—'}</strong></div>
  `;
  modal.style.display = 'flex';
}

function closeWithdrawDemandModal() {
  const modal = document.getElementById('withdraw-demand-modal');
  if (modal) modal.style.display = 'none';
  _withdrawRowRef = null;
}

function confirmWithdrawDemand() {
  if (!_withdrawRowRef) return;
  const row = _withdrawRowRef;
  const now = new Date();
  row.status = '已撤回';
  row.withdrawn_at = `${now.getFullYear()}/${String(now.getMonth() + 1).padStart(2, '0')}/${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
  row.withdraw_reason = '用户主动撤回后重新创建';
  row.resubmit_from = `${row.sku} · ${row.submit_time || row.date || '原需求'}`;
  if (typeof applyFilters === 'function') applyFilters();
  else if (typeof renderListTable === 'function') renderListTable();
  closeWithdrawDemandModal();
  prefillWizardFromWithdrawnDemand(row);
  showToast('需求已撤回，已带入原信息重新创建', 'success');
}

function prefillWizardFromWithdrawnDemand(row) {
  if (!row) return;
  if (typeof goToCreate === 'function') goToCreate({ silent: true });
  if (typeof setStep === 'function') setStep(1);

  applyWithdrawReqType(row.type);
  setFormValue('site', String(row.site || '').toLowerCase());
  applyWithdrawSubcategory(row.sub);
  applyWithdrawSku(row.sku);
  setFormValue('delivery-date', toDateInputValue(row.date));
  setFormValue('product-launch-date', toDateInputValue(row.launch_date));
  const remark = document.getElementById('demand-remark');
  if (remark) {
    remark.value = `由已撤回需求重新创建：${row.sku} · ${row.submit_time || row.date || '原需求'}`;
  }
  if (typeof updateProductLaunchField === 'function') updateProductLaunchField();
  if (typeof checkDateConsistency === 'function') checkDateConsistency();
  if (typeof saveStep1Form === 'function') saveStep1Form();
}

function applyWithdrawReqType(type) {
  const key = TYPE_TO_RESULT_KEY[type] || '';
  const parts = key.split('-');
  if (parts.length < 2) return;
  currentStage = parts[0];
  currentBiz = parts[1];
  currentSub = parts[2] || null;
  if (typeof renderBizGrid === 'function') renderBizGrid();
  if (typeof renderSubGrid === 'function') renderSubGrid();
  if (typeof syncReqType === 'function') syncReqType();
  document.querySelectorAll('.stage-tab').forEach(t => {
    t.classList.toggle('active', t.dataset.stage === currentStage);
  });
}

function applyWithdrawSubcategory(label) {
  const hidden = document.getElementById('subcategory');
  if (!hidden) return;
  const matched = (typeof subCategoriesData !== 'undefined' ? subCategoriesData : [])
    .find(item => item.label === label || item.value === label);
  hidden.value = matched ? matched.value : '';
  if (typeof renderSubcategoryTrigger === 'function') renderSubcategoryTrigger();
  if (typeof clearError === 'function') clearError(hidden);
}

function applyWithdrawSku(code) {
  if (!code) return;
  skuPool = (typeof allSkusData !== 'undefined' ? allSkusData : []).slice();
  skuList = [code];
  ...
  const total = document.getElementById('sku-pool-total');
  if (total) total.textContent = skuPool.length;
  const error = document.getElementById('sku-error');
  if (error) error.classList.remove('show');
  if (typeof renderSkuOptions === 'function') renderSkuOptions();
  if (typeof renderSkuTrigger === 'function') renderSkuTrigger();
}
