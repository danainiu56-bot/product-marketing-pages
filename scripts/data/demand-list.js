...
function getListingCharLimit(tag) {
  return /^Title$/i.test(String(tag || '')) ? 200 : 500;
}

function highlightListingSeoText(text) {
  if (!text) return '';
  ...
  [...LISTING_SEO_KEYWORDS]
    ...
    .forEach(kw => {
      ...
      html = html.replace(new RegExp(`(${escaped})`, 'gi'), '<mark class="listing-seo-highlight">$1</mark>');
    });
  return html;
}

function buildListingPointsHtml(text, options = {}) {
  const { highlightSeoTag = '' } = options;
  ...
  const cards = sections.map(sec => {
    ...
    if (!m) {
      ...
      return `<div class="copy-draft-listing-point">
        <div class="listing-point-main">
          <div class="listing-point-head">
            <span class="listing-point-tag">内容</span>
            <span class="listing-point-char">${count} 字符</span>
          </div>
          <div class="listing-point-text">${sec}</div>
        </div>
      </div>`;
    }
    const tag = m[1];
    ...
    const limit = getListingCharLimit(tag);
    const overClass = count > limit ? ' over-limit' : '';
    const shouldHighlightSeo = highlightSeoTag === '*' || (highlightSeoTag && tag === highlightSeoTag);
    const display = shouldHighlightSeo ? highlightListingSeoText(body) : body;
    return `<div class="copy-draft-listing-point">
      <div class="listing-point-main">
        <div class="listing-point-head">
          <span class="listing-point-tag">${tag}</span>
          <span class="listing-point-char${overClass}">${count}/${limit} 字符</span>
        </div>
        <div class="listing-point-text">${display}</div>
      </div>
    </div>`;
  }).join('');
...
${brand} 品牌包装完整，附带说明与售后支持，适合作为家庭健康管理或关怀礼品。`;
    const htmlContent = buildListingPointsHtml(content, { highlightSeoTag: '*' });
